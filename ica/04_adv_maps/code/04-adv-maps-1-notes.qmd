...
# Two new maps using chosen CRSs
transform_and_plot(nc, new_crs = "EPSG:2229", title = "California CRS")
transform_and_plot(nc, new_crs = "ESRI:102903", title = "Chicago CRS")
```
...
> I would expect very little accuracy, as depending on the size of the CRS, the map of North Carolina may appear smaller or larger than it actually is. Case in point, California's CRS making North Carolina looking really large, and Chicago's really small.
...
```{r}
# Checking for CRS of the aforementioned datasets.
st_crs(mn_cities)
st_crs(mn_water)
st_crs(mn_counties) # This dataset does not have the same CRS as the others; transformation is necessary.

# Updating 'mn_counties' CRS to match the other datasets'.
...
> The advantage is to allow color-blind people to be able to distinguish colors better, by picking a palatte that can be more accessible

- Look through the examples section. What is the difference between the `_d()`, `_c()`, and `_b()` variants of this function?

> The difference is what the palette is being applied to. '_d()' is for discrete data, '_c()' is for continuous data, and '_b()' is for pre-mapped continuous data.
