...
```{r}
# Compile Results
dplyr::bind_rows(results1, results2, results3, results4) |>
  dplyr::mutate(type = c("for_loop_no_storage", "for_loop_with_storage", "purrr_map", "standard_func")) |>
  dplyr::select(type, mem_alloc, total_time)  |> print()
...
map_chr(ggplot2::diamonds, class)
```
...
map_dbl(ggplot2::diamonds |> dplyr::select(where(is.numeric)), mean)

# Number of unique values in each non-numeric variable
map_int(ggplot2::diamonds |> dplyr::select(!where(is.numeric)), dplyr::n_distinct)
```
...
sim_data <- args |>
  dplyr::mutate(rand_data = pmap(args, rnorm))
...
unnested_sim <- sim_data |> 
  tidyr::unnest(rand_data) # convert the rand_data list to numeric vector
  
  
unnested_sim |> 
  ggplot2::ggplot(ggplot2::aes(x = rand_data, group = mean, fill = factor(mean))) +
...
```{r}
xs <- ggplot2::diamonds |> dplyr::select(carat, cut, clarity, color) |> names()
...
by_clarity <- ggplot2::diamonds |> 
  dplyr::group_nest(clarity)
...
by_clarity <- by_clarity |> 
  dplyr::mutate(path = str_glue("diamonds-{clarity}.png"))
...
carat_scatter <- function(df) {
  ggplot2::ggplot(df, ggplot2::aes(x = carat, y = price)) + ggplot2::geom_point()  
}

# Create a "variable" with each plots
by_clarity <- by_clarity |> 
  dplyr::mutate(plot = map(data, carat_scatter)) # iterate over data to create ggplot output
```
...
walk2( 
  by_clarity$path,
  by_clarity$plot,
  function(path, plot){ ggplot2::ggsave(path, plot, width = 6, height = 6)}
)
