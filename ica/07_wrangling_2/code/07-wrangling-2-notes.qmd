...
```{r ex_create_string}
# Your code
stringDouble <- "It's Thursday"
stringDouble

# stringSingle <- 'It's Thursday' 
# Because there is an extra single quote inside the actual string, the variable cannot be created
```
...
# Your code
stringUser <- "C:\Users"
stringUser # No hex digits in the character string.
```
...
middle <- str_sub(df, start = str_length(1:3)/2, end = str_length(1:3)/2)
middle
```
